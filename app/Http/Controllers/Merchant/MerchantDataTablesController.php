...
class ItemDataTablesController extends Controller {
    ...
    public function anydata(){
      ...
      ->editColumn('item', function ($item) {
        $status = $item->status ? '<i class="fa fa-circle text-success pull-right" title="Online"></i>' : '<i class="fa pull-right fa-circle text-muted" title="Offline"></i>';
           return '
            <div><b>' . $item->item . '</b> ' . $status . '</div>
            <small>[
                <a href="' . url($item->slug) . '" target="_blank" style="color:green;">Go Page</a> |
                <a style="color:green;" href="' . url('/item/' . $item->id_item . '') . '" title="Add service">Detail</a>
            ]</small>
           ';
       })
       ...
       ->editColumn('email', function ($item) {
        ...
        ->editColumn('premium', function ($item) {
            $checked = $item->premium ? 'checked' : '';
             return '<input type="checkbox" ' . $checked . ' name="premium" value="' . $item->id_item . '">';
         })
       ->editColumn('img_thumbnail', function ($item) {
            return '<img width="60" class="img-thumbnail" src="' . asset('/img/item/thumb/' . $item->img_thumbnail) . '">';
        })
        ->editColumn('id_item', function ($item) {
             return '<b>' . $item->id_item . '</b>';
         })
    ...
    public function services($id){
      ...
      ->editColumn('servicepack', function ($item) {
           return '
            <a href="' . url('/item/servicepack/' . $item->id_servicepack . '/edit') . '">' . $item->servicepack . '</a>
           ';
       })
    ...
    public function itemstatisticbooking(Request $req){
      if($req->ajax()){
        $res['result'] = true;
    ...
    public function itemstatisticincome(Request $req){
      if($req->ajax()){
        $res['result'] = true;
        $out = [];
    ...
    public function anyschedule(Request $req, $id){
      $items = data_schedule::where('id_item', $id)->select([
        'nm_schedule',
        'time_start',
        'status',
        'id'
      ...
      ->editColumn('nm_schedule', function ($item) {
           return '<a href="' . url('/item/schedule/' . $item->id . '/edit') . '">' . $item->nm_schedule . '</a>';
       })
